...
# ── Built-in symbol codes (from CGROM table) ──────────────────────────────────
# MSB = high nibble, LSB = low nibble of the byte
# e.g. MSB=0001, LSB=0000 → 0x10
SYMBOLS = {
    # Blocks and fills
    'BLOCK_FULL'      : 0xFF,  # solid filled block
    'BLOCK_HALF_LEFT' : 0x10,  # left half block
    'BLOCK_HALF_BOT'  : 0x11,  # bottom half block
    'BLOCK_SMALL'     : 0x12,  # small filled block
    'BLOCK_CHECKER'   : 0x0D,  # checkerboard pattern
    'BLOCK_CHECKER2'  : 0x0E,  # alternate checkerboard

    # Arrows
    'ARROW_RIGHT'     : 0x10,  # ▶ right pointing (play-like)
    'ARROW_LEFT'      : 0x11,  # ◀ left pointing
    'ARROW_UP'        : 0x18,  # ▲ up
    'ARROW_DOWN'      : 0x19,  # ▼ down

    # Media controls (using arrow-like symbols from ROM)
    'PLAY'            : 0x10,  # ▶
    'PAUSE'           : 0x0F,  # ⏸ (two vertical bars)
    'STOP'            : 0xFF,  # solid block ■
    'SKIP_RIGHT'      : 0x1A,  # ⏭
    'SKIP_LEFT'       : 0x1B,  # ⏮

    # Punctuation and misc from extended set
    'DEGREE'          : 0xDF,  # °
    'DIVIDE'          : 0xFD,  # ÷
    'MULTIPLY'        : 0xD6,  # ×
    'ALPHA'           : 0xE0,  # α
...
# ── Progress bar partial fill bitmaps (CGRAM slots 0–3) ──────────────────────
# 5 columns per character, LSB = top row. Loaded on demand.
_BAR_BITMAPS = [
    [0x1F, 0x00, 0x00, 0x00, 0x00],  # slot 0 — 1/5 fill
    [0x1F, 0x1F, 0x00, 0x00, 0x00],  # slot 1 — 2/5 fill
    [0x1F, 0x1F, 0x1F, 0x00, 0x00],  # slot 2 — 3/5 fill
    [0x1F, 0x1F, 0x1F, 0x1F, 0x00],  # slot 3 — 4/5 fill
]
_BAR_SLOTS_LOADED = False
...
class FutabaVFD:
    ...
    def progress_bar(self, fraction, prefix='', suffix='',
        ...
        fine       : if True, use CGRAM partial fill for smooth sub-cell steps
                     (uses CGRAM slots 0–3). If False, use plain characters only.
        empty_char : character for empty cells (default '-')

        Examples:
          vfd.progress_bar(0.5)
          vfd.progress_bar(0.75, prefix='[', suffix=']')
          vfd.progress_bar(0.5, fine=False, empty_char='.')
        """
        ...
        if fine:
            # Load partial fill bitmaps on first use
            if not self._bar_loaded:
                self._load_bar_bitmaps()

            total_units  = bar_cells * 5
            filled_units = round(fraction * total_units)
            full_cells   = filled_units // 5
            remainder    = filled_units % 5

            for i in range(bar_cells):
                if i < full_cells:
                    self._write_cmd([_DCRAM_DATA_WRITE | pos, 0xFF])  # solid block
                elif i == full_cells and remainder > 0:
                    self._write_cmd([_DCRAM_DATA_WRITE | pos, remainder - 1])  # CGRAM slot 0–3
                else:
                    self._write_cmd([_DCRAM_DATA_WRITE | pos, ord(empty_char)])
                pos += 1
        else:
            filled = round(fraction * bar_cells)
            for i in range(bar_cells):
                if i < filled:
                    self._write_cmd([_DCRAM_DATA_WRITE | pos, 0xFF])
                else:
                    self._write_cmd([_DCRAM_DATA_WRITE | pos, ord(empty_char)])
                pos += 1
    ...
    def _load_bar_bitmaps(self):
        """Load partial fill bitmaps into CGRAM slots 0–3."""
...
if __name__ == "__main__":
    ...
    with FutabaVFD(din=21, clk=20, cs=16, rst=26, en=19) as vfd:
        ...
        print("Fine progress bar...")
        vfd.progress_bar_animated(0.0, 1.0, duration=3.0, prefix='[', suffix=']', fine=True)

        print("Simple progress bar...")
        vfd.progress_bar_animated(0.0, 1.0, duration=2.0, prefix='[', suffix=']', fine=False)
