...
class MenuState:
    ...
    def _play(self, filepath):
        """
        Called when the user presses Enter on a file.
        For now just prints -- wire this up to your actual player
        (e.g. send the path over a socket to a player daemon, or shell out).
        """
        print(f"[vfd_menu] play requested: {filepath}")
        # TODO: hook up to the real music player script/daemon
    ...
    def _start_scroll(self, vfd, text):
        ...
        def loop():
            ...
            while not self._scroll_stop.is_set():
                ...
                time.sleep(SCROLL_PAUSE)  # brief pause before looping again
