...
class MenuState:
    ...
    def enter(self, vfd):
        with self.lock:
            ...
            if is_dir:
                ...
                # It's a file -- stop any active scroll first, then send one
                # final static frame so the menu's stored claim isn't left on
                # a half-finished scroll frame (which would reappear frozen
                # if the player later releases the display back to the menu).
                self._stop_scroll()
                vfd.write_str(0, name[:8].ljust(8))
                self._play(full)
    ...
    def refresh(self, vfd):
        """
        Re-scan the CURRENT folder without navigating anywhere -- picks up
        any new files/folders added since the menu last loaded this
        directory (e.g. after sync_music.py finishes). Tries to keep the
        same item selected if it still exists.
        """
        with self.lock:
            current_name = None
            if self.entries:
                current_name = self.entries[self.selected][0]

            self.entries = list_dir(self.current_path)

            if current_name is not None:
                try:
                    self.selected = [e[0] for e in self.entries].index(current_name)
                except ValueError:
                    self.selected = 0
            else:
                self.selected = 0
