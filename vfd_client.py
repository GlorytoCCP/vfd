...
class VFDClient:
    ...
    def scroll_h(self, text, delay=0.15, loops=1, pad=True, wait=True):
        self._push([["scroll_h", [text, delay, loops, pad]]])
        if wait:
            duration = (len(text) + (8 if pad else 0)) * delay * loops
            time.sleep(duration + 0.5)

    def scroll_v(self, position, chars, delay=0.1, loops=1, wait=True):
        self._push([["scroll_v", [position, chars, delay, loops]]])
        if wait:
            duration = (len(chars) - 1) * 8 * delay * loops
            time.sleep(duration + 0.5)

    def blink(self, times=3, on_time=0.3, off_time=0.2, wait=True):
        self._push([["blink", [times, on_time, off_time]]])
        if wait:
            duration = (on_time + off_time) * times
            time.sleep(duration + 0.5)

    def blink_position(self, position, code, times=3, on_time=0.3, off_time=0.2, wait=True):
        self._push([["blink_position", [position, code, times, on_time, off_time]]])
        if wait:
            duration = (on_time + off_time) * times
            time.sleep(duration + 0.5)

    def progress_bar(self, fraction, prefix="", suffix="", wait=False, hold=0.0):
        """
        wait : if True, sleep briefly so this frame has time to actually
               render before the next call -- prevents rapid successive
               calls from having frames silently dropped by the manager's
               queue (which always keeps only the latest pending update).
        hold : explicit number of seconds to wait, overriding the default
               small delay used when wait=True.
        """
        self._push([["progress_bar", [fraction, prefix, suffix]]])
        if wait:
            time.sleep(hold if hold > 0 else 0.05)

    def progress_bar_animated(self, start, end, duration=2.0,
                               prefix="", suffix="", steps=40, wait=True):
    ...
    def reclaim(self):
        """
        Resume using this client after a previous release() call.
        Needed because release() permanently disables _push() otherwise --
        long-lived daemons (like player.py) that repeatedly stop/start
        should call this before sending new content, rather than creating
        a brand new VFDClient each time.
        """
