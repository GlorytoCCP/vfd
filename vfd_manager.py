...
class VFDManager:
    ...
    def _claim(self, msg):
        ...
        with self.claims_lock:
            ...
            # Only force replay if this claiming client is actually the top
            # priority owner — a lower-priority claim arriving should never
            # restart a one-shot animation belonging to a higher-priority owner.
            if self.claims:
                _, top_claim = max(self.claims.items(),
                                   key=lambda kv: kv[1].priority)
                is_top = top_claim.client_id == client_id
            else:
                is_top = False

            self._enqueue_refresh(force_replay=is_top)
        return {"ok": True}
